...
app.get('/api/audit/summary', (c) => {
  try {
    ...
    const compositeParams = db.prepare(`
...
async function runPythonCommand(args: string[], cwd: string): Promise<{stdout: string, stderr: string, exitCode: number}> {
  if (typeof process !== 'undefined' && process.versions && (process.versions as any).bun) {
    // @ts-ignore
