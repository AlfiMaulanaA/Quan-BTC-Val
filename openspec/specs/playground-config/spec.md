...
## Purpose
Defines the storage and retrieval mechanism for threshold configuration parameters of the 17 supported valuation metrics, supporting database pre-seeding, user-defined updates, and bulk config loading.

## Requirements

### Requirement: Pre-Seed All Metric Configs

The system MUST pre-seed the `metric_config` table with threshold values for all 17 supported metrics during database initialization. The pre-seeded metric names and their threshold values SHALL match the expert-defined values:
